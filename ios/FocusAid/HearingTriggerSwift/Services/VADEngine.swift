...
public final class VADEngine {
    ...
    private var cooldownMs: Int = 6000
    private var capturePostMs: Int = 3000
    ...
    private let ENERGY_THRESHOLD: Float = 0.045  // normalized RMS — filters room noise, TV hum
    ...
    private var preRollSamples = [Int16]()       // 0.5s sliding pre-roll
    ...
    public func start(threshold: Float,
                      cooldownMs: Int,
                      capturePostMs: Int,
                      onClipCaptured: @escaping (URL) -> Void,
                      onError: @escaping (Error) -> Void,
                      onLogging: ((String) -> Void)? = nil) {
        self.threshold = threshold
        self.cooldownMs = cooldownMs
        self.capturePostMs = capturePostMs
        ...
        do {
            ...
            _isListening = true
            log("VAD Engine started successfully.")
        } catch {
            _isListening = false
            onError(error)
        }
    ...
    public func stop() {
        ...
        log("VAD Engine stopped.")
    }
    
    public func updateConfig(threshold: Float, cooldownMs: Int, capturePostMs: Int) {
        self.threshold = threshold
        self.cooldownMs = cooldownMs
        self.capturePostMs = capturePostMs
        log("VAD Engine config updated: threshold=\(threshold), cooldownMs=\(cooldownMs), capturePostMs=\(capturePostMs)")
    }
    ...
    private func startAudioEngine() throws {
        ...
        guard let cvt = AVAudioConverter(from: hwFormat, to: targetFormat) else {
            throw NSError(domain: "VADEngine", code: -1,
    ...
    private func handleBuffer(_ buffer: AVAudioPCMBuffer, hwFormat: AVAudioFormat) {
        ...
        vadQueue.async { [weak self] in
            self?.processVAD(samples: samples, rms: rms, now: now)
        }
    }
        ...
        if !isRecording {
            ...
            if rms > ENERGY_THRESHOLD {
                ...
                if onsetMs >= MIN_SPEECH_MS && cooldownElapsed {
                    ...
                    log("Speech onset detected! Starting recording clip...")
                }
            } else {
                speechOnsetAt = nil
            }
            ...
            if elapsedMs >= Double(capturePostMs) {
                ...
                speechOnsetAt = nil
                preRollSamples = []
                log("Clip duration reached. Finalizing VAD detection.")
    ...
    private func finalizeDetection(samples: [Int16]) {
        if let fileURL = writeWAV(samples: samples) {
            onClipCaptured?(fileURL)
        } else {
            log("Error writing WAV file.")
        }
    }
    
    // ─── WAV writer ───────────────────────────────────────────────
    private func writeWAV(samples: [Int16]) -> URL? {
